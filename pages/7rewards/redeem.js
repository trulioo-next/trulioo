...
const Home = props => {
  ...
  const verifySMSWithCode = () => {
    dispatch(
      reqSMSAction({ token: user.token, mobileNumber: phone, code: code }),
    );
  };
  ...
  let redeemError =
    user && user.redeem && user.redeem.error
      ? user.redeem.error.payload.error_description
      : false;
  let smsError =
    user && user.sms && user.sms.error
      ? user.sms.error.payload.error_description
      : false;
  let smsSuccess =
    user && user.sms && user.sms.success ? user.sms.success : false;

  let redeemSuccessPoints =
    user && user.redeem && user.redeem.rewards_points
      ? user.redeem.rewards_points
      : false;
        ...
        <Container>
          <Row>
            <Col className="rewards-wrapper">
              ...
              <div>
                {redeemSuccessPoints && <h2>Points : {redeemSuccessPoints}</h2>}
              </div>
              ...
              {redeemError && <p className="error__message">{redeemError}</p>}
            </Col>
            <Col>
              ...
              <Button
                ...
                onClick={verifySMSWithCode}
              >
                Verify SMS
              </Button>

              {smsSuccess && <p className="error__message">{smsSuccess}</p>}

              {smsError && <p className="error__message">{smsError}</p>}
            </Col>
          </Row>
        </Container>
      </div>
