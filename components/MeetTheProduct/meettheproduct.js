              ...
              { component.products.map((product, index) =>
                (
                  <Card
                    ...
                    className={ classnames(
                      'd-flex border-0',
                      { 'opacity-100' : index >= active && index <= active + (visibleSlides - 1) },
                      { 'opacity-0' : index < active - 1 || index > active + visibleSlides }
                    ) }
                  >
                    <CardImg className="d-block" top width="100%" src={ product.image.url } alt={ product.image.alt } />
                    <CardBody>
                      <CardTitle tag="h3" className="mb-4">{ product.title }</CardTitle>
                      <ul className="d-block px-0">
                        { product.icons && product.icons.map((icon, index) =>
                          (
                            <li key={ index } className={ `d-inline-block ${ product.icons.length === index + 1 ? 'mr-0' : 'mr-3 mr-md-4' } pl-0` }>
                      ...
                      <CardText
                        ...
                        dangerouslySetInnerHTML={ { __html: product.paragraph || 'No HTML' } }
                      />
                    </CardBody>
                    { product.link && (
                      <CardFooter className="bg-transparent border-0">
                        <FlipButton
                          ...
                          href={ product.link.target ? product.link.url : new URL(product.link.url).pathname + new URL(product.link.url).search + new URL(product.link.url).hash }
                          target={ product.link.target }
                        >{product.link.title}</FlipButton>
                      </CardFooter>
                    )}
                  </Card>
                )
