...
export const OnPageNav = ({ components }) => {
        ...
        return (
          <li key={index} className="on-page-nav-item d-block">
            <a
              ...
              href={'#' + menu.anchor}
              onClick={event => jumpTo(`#${menu.anchor}`, event)}
            >
              {menu.icon && (
                <img
              ...
              <span className="d-block on-page-nav-item-label">
                {menu.nav_title
                  ? menu.nav_title
                  : new DOMParser().parseFromString(menu.title, 'text/html')
  ...
  return (
    <nav
      ref={ref}
      className="on-page-nav sticky-top"
      onMouseEnter={() => {
